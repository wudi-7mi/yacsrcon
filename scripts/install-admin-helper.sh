...
printf '%s ALL=(root) NOPASSWD: %s read, %s apply, %s bans, %s cfg, %s storage-status, %s server-status, %s server-logs, %s server-start, %s server-stop, %s server-restart\n' \
  "$app_user" "$helper" "$helper" "$helper" "$helper" "$helper" "$helper" "$helper" "$helper" "$helper" "$helper" > "$temporary"
