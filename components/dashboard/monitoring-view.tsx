...
import { Activity, AlertTriangle, Clock3, RefreshCw, Signal, Users } from "lucide-react";
import { Empty, PageTitle } from "@/components/dashboard/view-primitives";
import { useApiRequest } from "@/hooks/use-api-request";
import type { MetricSample, MonitoringResult } from "@/lib/types";
...
export function MonitoringView({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setData(await request<MonitoringResult>(`/api/monitoring?hours=${hours}`, { cache: "no-store" }));
      setError("");
    } catch (cause) {
  ...
  return <>
    ...
    {summary?.currentAlert && <div className="mb-4 flex items-center gap-3 rounded-lg border border-[#6a2930] bg-[#29161a] px-4 py-3 text-sm text-[var(--danger)]"><AlertTriangle size={17} />{summary.currentAlert === "disconnected" ? "当前 RCON 连接中断，请检查服务器进程与网络。" : "当前 RCON 延迟超过 500 ms。"}</div>}
