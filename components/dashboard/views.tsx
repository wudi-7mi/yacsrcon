export { AuditView } from "@/components/dashboard/audit-view";
export { ConsoleView } from "@/components/dashboard/console-view";
export { ConfigView } from "@/components/dashboard/config-view";
export { MapsView } from "@/components/dashboard/maps-view";
export { OverviewView } from "@/components/dashboard/overview-view";
export { PlayersView } from "@/components/dashboard/players-view";
export { PluginCenterView } from "@/components/dashboard/plugin-center-view";
export { MonitoringView } from "@/components/dashboard/monitoring-view";
export { ServerOperationsView } from "@/components/dashboard/server-operations-view";
